...
class Avatar:
    ...
    def action(self, percepts):
        ...
        if len(actions) != maxActions:
            if self.player.game.in_tournament:
                raise RuntimeError(
                    "Error! The returned action list from AgentFunction must contain %d items" % maxActions)
            else:
                print("Error! The returned action list from AgentFunction must contain %d items, not %d" % (
    ...
    def __init__(self, game, player, playerFile, nAgents, emptyMode=False, train_temp=False):
        ...
        if isinstance(self.trainingSchedule, list):
            ...
            for trainSession in self.trainingSchedule:
                if not isinstance(trainSession, tuple) or len(trainSession) != 2 or not isinstance(trainSession[0],
                ...
                if trainSession[1] < 0:
                    if self.game.in_tournament:
                        ...
                        print(
                            "Error! Agent's 'trainingSchedule' should be a list of (str,int) tuples, where int corresponds to the number of train generations.")
                        sys.exit(-1)
            ...
            if totTrainEpochs > maxTrainingEpochs:
                if self.game.in_tournament:
                    ...
                    print(
                        "Error! Agent's 'trainingSchedule' cannot specify more than %d training epochs in total." % maxTrainingEpochs)
                    sys.exit(-1)
        ...
        if not os.path.exists(savedAgent) or (
                not self.game.in_tournament and os.path.getmtime(savedAgent) < os.path.getmtime("%s.py" % agentFile)):
            agents = list()
        ...
        for agent in agents:
            if type(agent) != self.exec.MyCreature:
                if self.game.in_tournament:
                    ...
                    print(
                        "Error! The new_population returned form newGeneration() in '%s.py' must contain objects of MyCreature() type" %
                        self.playerFile)
                    traceback.print_exc()
                    sys.exit(-1)
    ...
    def new_generation_agents(self):
        ...
        if type(result) != tuple or len(result) != 2:
            if self.game.in_tournament:
                ...
                print(
                    "Error! The returned value form newGeneration() in '%s.py' must be a 2-item tuple" % self.playerFile)
                traceback.print_exc()
                sys.exit(-1)
        ...
        if type(new_population) != list:
            if self.game.in_tournament:
                ...
                print(
                    "Error! The new_population returned form newGeneration() in '%s.py' must be a list" % self.playerFile)
                traceback.print_exc()
                sys.exit(-1)
        ...
        try:
            ...
            if self.game.in_tournament:
                ...
                print(
                    "Error! The new_population returned form newGeneration() in '%s.py' must be a float or int" % self.playerFile)
                traceback.print_exc()
                sys.exit(-1)
        ...
        if len(new_population) != len(old_population):
            if self.game.in_tournament:
                raise RuntimeError(
                    'Error! The new_population returned form newGeneration() must contain %d items' % self.nAgents)
            else:
                print("Error! The new_population returned form newGeneration() in '%s.py' must contain %d items" % (
...
class Game:
    ...
    def init_agent_and_food_maps(self, players):
        ...
        for n in range(self.nAgents):
            while True:
                ...
                position_player_1 = np.array([int(range_mid + rand_offset[0]), int(range_mid + rand_offset[1])]).astype(
                    'int')
                position_player_2 = np.array([int(range_mid - rand_offset[0]), int(range_mid - rand_offset[1])]).astype(
                    'int')
        ...
        for n in range(self.nAgents):
            while True:
                ...
                position_food_1 = np.array([int(range_mid + rand_offset[0]), int(range_mid + rand_offset[1])]).astype(
                    'int')
                position_food_2 = np.array([int(range_mid - rand_offset[0]), int(range_mid - rand_offset[1])]).astype(
                    'int')
    ...
    def play(self, players, show_games, save_games, visResolution=(720, 480), visSpeed='normal', savePath="saved",
             trainGames=0):

        # Create the visualiser
        if len(show_games) > 0 and not self.in_tournament:
            import vis_pygame as vis
            self.vis = vis.visualiser(speed=visSpeed, gridSize=self.gridSize,
                                      playerStrings=(players[0].name, players[1].name),
        ...
        for game in range(1, nPlays + 1):
            ...
            for turn in range(self.nTurns):
                ...
                if game in show_games and not self.in_tournament:
                    ...
                    self.vis.show(creature_state=vis_agents[:, :, turn], food_array=vis_food[turn],
                                  wall_array=vis_walls, game=gameCaption, turn=turn)
            ...
            if game in show_games and not self.in_tournament:
                self.vis.show(creature_state=vis_agents[:, :, turn + 1], food_array=vis_food[turn + 1],
                              wall_array=vis_walls, game=game, turn=turn + 1)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "p:r:v:s:f:l:g:",
                                   ["players=", "res=", "vis=", "save=", "fast=", "load=", "games="])
    ...
    for opt, arg in opts:
        if opt in ("-p", "--players"):
            ...
            if len(players) != 2:
                print(
                    "Error! The -p/players= argument must be followed with two comma separated file name (no spaces).")
                sys.exit(-1)
            ...
            if len(res) != 2:
                print(
                    "Error! The -r/res= argument must be followed with <width>x<height> specifiction of resolution (no spaces).")
                sys.exit(-1)
...
def avg_fitness_array(avg_fitness):
    global avg_fit, nGenerations
    avg_fit.append(avg_fitness)
    nGenerations.append(len(avg_fit))
...
if __name__ == "__main__":
    ...
    print(nGenerations)
    print(avg_fit)
    plt.plot(nGenerations, avg_fit)
    plt.title("Average Fitness against RandomPlayer")
